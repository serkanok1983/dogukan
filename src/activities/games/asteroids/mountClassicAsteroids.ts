...
export function mountClassicAsteroids(
  ...
  const createBelt = () => {
    ...
    for (let i = 0; i < ROID_NUM + level; i++) {
      ...
      do {
        x = Math.random() * W;
        y = Math.random() * H;
      } while (dist(ship.x, ship.y, x, y) < ROID_SIZE * 2 + ship.r);
      roids.push(newAsteroid(x, y, Math.ceil(ROID_SIZE / 2)));
    }
  };

  const drawShip = (x: number, y: number, a: number, colour = "white") => {
  ...
  const keyDown = (ev: KeyboardEvent) => {
    ...
    if (ev.keyCode === 32) shootLaser();
    if (ev.keyCode === 37) ship.rot = (SHIP_TURN_SPD / 180) * (Math.PI / FPS);
    if (ev.keyCode === 38) ship.thrusting = true;
    if (ev.keyCode === 39) ship.rot = (-SHIP_TURN_SPD / 180) * (Math.PI / FPS);
  };

  const keyUp = (ev: KeyboardEvent) => {
    if (ship.dead) return;
    if (ev.keyCode === 32) ship.canShoot = true;
    if (ev.keyCode === 37 || ev.keyCode === 39) ship.rot = 0;
    if (ev.keyCode === 38) ship.thrusting = false;
  };
  ...
  const update = () => {
    ...
    if (!exploding) {
      ...
      const scales = [1.7, 1.4, 1.1, 0.8, 0.5];
      colors.forEach((c, i) => {
        ctx.fillStyle = c;
        ctx.beginPath();
        ctx.arc(ship.x, ship.y, ship.r * scales[i], 0, Math.PI * 2);
        ctx.fill();
      });
    }
    ...
    for (const laser of ship.lasers) {
      if (laser.explodeTime === 0) {
        ...
        ctx.beginPath();
        ctx.arc(laser.x, laser.y, SHIP_SIZE / 15, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    ...
    for (let i = 0; i < lives; i++) {
      drawShip(SHIP_SIZE + i * SHIP_SIZE * 1.2, SHIP_SIZE, 0.5 * Math.PI, exploding && i === lives - 1 ? "red" : "white");
    }

    ctx.textAlign = "right";
    ctx.fillStyle = "white";
    ctx.font = `${TEXT_SIZE}px var(--font-nunito), monospace`;
    ctx.fillText(String(score), W - SHIP_SIZE / 2, SHIP_SIZE);
    ctx.textAlign = "center";
    ctx.font = `${TEXT_SIZE * 0.75}px var(--font-nunito), monospace`;
    ctx.fillText(`EN İYİ ${scoreHigh}`, W / 2, SHIP_SIZE);

    for (let i = roids.length - 1; i >= 0; i--) {
      const roid = roids[i];
      for (let j = ship.lasers.length - 1; j >= 0; j--) {
        const laser = ship.lasers[j];
        if (laser.explodeTime === 0 && dist(roid.x, roid.y, laser.x, laser.y) < roid.r) {
          destroyAsteroid(i);
    ...
    if (!exploding && ship.blinkNum === 0 && !ship.dead) {
      for (let i = 0; i < roids.length; i++) {
        if (dist(ship.x, ship.y, roids[i].x, roids[i].y) < ship.r + roids[i].r) {
          explodeShip();
          destroyAsteroid(i);
          break;
        }
    ...
    const wrap = (v: number, max: number, r: number) => {
      if (v < -r) return max + r;
      if (v > max + r) return -r;
      return v;
    };
    ship.x = wrap(ship.x, W, ship.r);
    ship.y = wrap(ship.y, H, ship.r);
    ...
    for (const roid of roids) {
      roid.x += roid.xv;
      roid.y += roid.yv;
      roid.x = wrap(roid.x, W, roid.r);
      roid.y = wrap(roid.y, H, roid.r);
    }
  };
